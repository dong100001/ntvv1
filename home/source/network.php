...
$query = $_SGLOBAL['db']->query("SELECT * FROM ".tname('feed')." WHERE friend='0' ORDER BY dateline DESC LIMIT 0,7");
...
if(check_network_cache('thread')) {
	...
	$query = $_SGLOBAL['db']->query("SELECT main.*, m.tagname
		...
		ORDER BY main.{$order} $sc LIMIT 0,$shownum");
	while ($value = $_SGLOBAL['db']->fetch_array($query)) {
		$value['tagname'] = getstr($value['tagname'], 10);
		$value['subject'] = getstr($value['subject'], 25);
		$threadlist[] = $value;
	}
...
$query = $_SGLOBAL['db']->query("SELECT sh.note, s.* FROM ".tname('show')." sh
	...
	ORDER BY sh.credit DESC LIMIT 0,7");
while ($value = $_SGLOBAL['db']->fetch_array($query)) {
	realname_set($value['uid'], $value['username'], $value['name'], $value['namestatus']);
