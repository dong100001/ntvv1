...
# QFarm Interface For UCHome 2.0
...
function qf_getTName($name, $tbpre = '') {
	return $GLOBALS['_QSC']['db']['tbprefix'] . $name;
}
...
function qf_userCredit($uid = 0, $credit = null) {
	...
	$uid = $uid > 0 ? $uid : $_QFG['uid'];
	if($credit === null) {//获取积分
		return (int)$_QFG['db']->result("SELECT credit FROM pap_tbpre_space where uid={$uid}");
	}
	else {//修改积分
		return $_QFG['db']->query("UPDATE pap_tbpre_space set credit=credit{$credit} where uid={$uid}");
	}
}
