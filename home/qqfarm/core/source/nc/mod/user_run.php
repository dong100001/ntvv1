...
if($toFriend) {
	$list = $_QFG['db']->fetchOne('SELECT C.uid,C.username,C.regname,C.money,C.pf,C.yb,C.tianqi,N.status,N.reclaim,N.exp,N.taskid,N.badnum,N.dog,N.decorative,N.activeItem,N.healthMode,N.chris,N.randtime FROM  app_qqfarm_user C Left JOIN app_qqfarm_nc N ON N.uid=C.uid where C.uid=' . $uId);
} else {
	$list = $_QFG['db']->fetchOne('SELECT C.money,C.username,C.regname,C.uid,C.yb,C.vip,C.pf,C.tianqi,N.status,N.reclaim,N.nc_e,N.exp,N.taskid,N.badnum,N.dog,N.decorative,N.activeItem,N.healthMode,N.randtime FROM app_qqfarm_user C Left JOIN app_qqfarm_nc N ON N.uid=C.uid where C.uid=' . $uId);
}
...
foreach($status as $key => $value) {
	...
	if($status[$key]['a'] != 0) {
		...
		if($zuowutime >= $cropstype[$a]['growthCycle'] && $k == 0 && $q != 0) {
			...
			$l = floor($k * 0.6);
			if( $status[$key]['bitmap'] == 1 && $status[$key]['a'] < 200){
				$k = intval($k * 1.1);
			}
			$m = $k;
			$status[$key]['b'] = $b;
...
//广告牌
if($list['activeItem'] > 0) {
	$decorative_echo['9']['itemId'] = $list['activeItem'];
}
...
if($toFriend) {
	...
	isset($npc) || $npc = '';
	//返回信息
	echo '{"a":0,"c":0,"dog":' . qf_jsonCode($dogstr) . ',"exp":' . $list['exp'] . ',"farmlandStatus":' . qf_jsonCode($status) . ',"items":' . qf_jsonCode($decorative_echo) . $npc.',"user":{"healthMode":{"beginTime":' . $healthMode['beginTime'] . ',"canClose":' . $canClose . ',"date":"' . $healthMode['date'] . '","endTime":' . $healthMode['endTime'] . ',"serverTime":' . $_QFG['timestamp'] . ',"set":' . $set . ',"time":"' . $healthMode['time'] . '","valid":' . $valid . '},"pf":' . $list['pf'] . '}}';
} else {
	//使坏次数
	$canbad = $list['badnum'];
	//消费提示
	...
	//返回信息
	echo '{"a":' . $a . ',"b":1,"c":' . $c . ',"d":' . (int)$vip['gift_nc'] . ',"dog":' . qf_jsonCode($dogstr) . ',"e":' . $list['nc_e'] . ',"exp":' . $list['exp'] . ',"farmlandStatus":' . qf_jsonCode($status) . ',"items":' . qf_jsonCode($decorative_echo) . ',"serverTime":{"time":' . $_QFG['timestamp'] . '}' . $taskid . ',"user":{"canbad":' . $canbad . ',"exp":' . $list['exp'] . ',"headPic":"' . qf_getheadPic(0, 'small') . '","healthMode":{"beginTime":' . $healthMode['beginTime'] . ',"canClose":' . $canClose . ',"date":"' . $healthMode['date'] . '","endTime":' . $healthMode['endTime'] . ',"serverTime":' . $_QFG['timestamp'] . ',"set":' . $set . ',"time":"' . $healthMode['time'] . '","valid":' . $valid . '},"missionTime":' . $missionTime . ',"money":' . $list['money'] . ',"pf":' . $list['pf'] . ',"uId":' . $_QFG['uid'] . ',"uinLogin":10000,"userName":"' . $username . '","yellowlevel":' . qf_toVipLevel($vip['exp']) . ',"yellowstatus":' . (int)$vip['status'] . '},"weather":{"weatherDesc":"' . $tqq . '","weatherId":' . $list['tianqi'] . '}}';
}
