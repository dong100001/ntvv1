...
$log = '[' . implode(',', $log) . ']';
//标记已读
$_QFG['db']->query("UPDATE app_qqfarm_nclogs set isread=1 where uid=" . $uId);
...
foreach($values as $value) {
	if($value['fromid'] == $value['toid']) {
		$value["fromname"] = $value["toname"] = "自己";
	}
	if($chat) {
...
//标记已读
$_QFG['db']->query("UPDATE app_qqfarm_message set isread=1 where toid=" . $uId);
