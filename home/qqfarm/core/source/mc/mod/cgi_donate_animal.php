...
$list = $_QFG['db']->fetchOne("SELECT status FROM app_qqfarm_mc where uid=" . $_QFG['uid']);
...
//消费日志
$values = $_QFG['db']->fetchAll("SELECT * FROM app_qqfarm_mclogs WHERE uid = " . intval($_QFG['uid']) . " AND type = 12 AND time > " . ($_QFG['timestamp'] - 3600) . " AND fromid =" . $_QFG['uid']);
