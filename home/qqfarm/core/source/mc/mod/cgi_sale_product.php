...
if((int)$_REQUEST['saleAll'] == '1') {
	...
	//出售日志
	$values = $_QFG['db']->fetchAll("SELECT * FROM app_qqfarm_mclogs WHERE uid = " .intval($_QFG['uid']) . " AND type = 9 AND time > " . ($_QFG['timestamp'] -3600) . " AND fromid =" . $_QFG['uid']);
	foreach($values as $value) {
		if (($value['type'] == 9) && ($value['fromid'] == $_QFG['uid']) && ($num > 0)) {
			...
			$moneyt = $moneyt + $money;
			$sql1 = "UPDATE app_qqfarm_mclogs set iid = '" . $list ."', money = '" . $moneyt . "', count ='" . $scount . "', time = " . $_QFG['timestamp'] .", isread = 1 where uid = " . intval($_QFG['uid']) ." AND type = 9 AND time > " . ($_QFG['timestamp'] - 3600) . " AND fromid =" .$_QFG['uid'];
